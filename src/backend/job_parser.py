...
4. 仅从明确条件提取学历、毕业届次、地点、每周天数与实习时长。graduation_cohorts 只填明确的 20XX 届/毕业年份；“应届”“全体在校生”“接受大一大二”不是毕业届次。未写出的到岗窗口、出差、加班、工作强度、任务频率和自主程度保持 unknown。
5. 职责里会用到的工具标为 role_context；任职要求明确写“掌握 SQL、Python 和 A/B 实验”等且无“等/或/优先”时，应按必须要求，不能一律标 uncertain；若只是“SQL、Python、R 等工具”举例，逐项强度保持 uncertain。'至少 X-Y 天'不能理解为最多 Y 天。只有明确写出更长时长优先，才填写 preferred。
6. 岗位主职能由真实任务决定；金融公司只提供行业背景，不能使金融工作相关度高于 1，除非职责确实涉及金融任务。使用 AI 工具不自动等于 AI 产品，行业洞察不自动等于纯研究。
...
8. 案例SOP或模板沉淀与伙伴培训/技术支持代表不同工作偏好，应分别建 task_clusters。不能把“愿意实际操作工具”写成已经具备工具操作能力；不能从“主动”推断独立负责，也不能从“低代码”推断完全不需编程。客户现场频率和出差仍可能未知。
